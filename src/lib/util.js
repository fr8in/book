...
var util = {
    ...
    handleLengthCheck: function (e) {
        if (e.target.value.length > e.target.maxLength) {
            e.target.value = e.target.value.slice(0, e.target.maxLength);
        }
    },
    membership_color: function (membership) {
        return ((membership === 1 || !membership) ? '#C0C0C0' : membership === 2 ? '#FFD700' : '#97b9ff');
    },
    membership: function (membership) {
        return ((membership === 1 || !membership) ? 'Silver' : membership === 2 ? 'Gold' : 'Platinum');
    },
    fileType: {
        pod: 'POD',
        lr: 'LR',
    ...
    is_roles: function (allowed_roles, context) {
        var result = context.roles.some(function (role) { return allowed_roles.includes(role); });
        return result;
    }
};
console.log(util.getLastSixMonth());
